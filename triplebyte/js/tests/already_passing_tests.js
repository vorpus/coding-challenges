...
function crawler_tests() {
    describe('test crawl triplebyte.com', () => {
        const crawler = new WebCrawler(100, SilentCrawlerLogger);

        it('passes', function(done) {
            this.timeout(10000);

            crawler.crawl('https://www.triplebyte.com', graph => {
                assert.notEqual(-1, Object.keys(graph.nodes).indexOf('https://www.triplebyte.com'));

                assert.notEqual(-1, Object.keys(graph.nodes).indexOf('https://www.triplebyte.com/careers'));

                assert.equal(graph.nodes['http://www.olark.com/?welcome'].request_type, 'head');
